...
class TaskListBloc extends Bloc<TaskEvent, TaskListState> {
  ...
  void _onLoadTasks(LoadTasksEvent event, Emitter<TaskListState> emit) {
    ...
    _taskStreamSubscription = taskRepository.watchTasks(
      page: 1,
      ...
      onError: (error, stackTrace) {
        log('[TaskListBloc] Stream error: $error', error: error, stackTrace: stackTrace);
  ...
  void _onLoadMoreTasks(LoadMoreTasksEvent event, Emitter<TaskListState> emit) async {
    log('[TaskListBloc] Loading more tasks...');
    
    // Validações
    if (_isLoadingMore || !state.hasMorePages || state.isLoading) {
      log('[TaskListBloc] Cannot load more: isLoadingMore=$_isLoadingMore, hasMorePages=${state.hasMorePages}, isLoading=${state.isLoading}');
      return;
    }
    ...
    try {
      ...
      log('[TaskListBloc] Error loading more tasks: $e', error: e, stackTrace: stackTrace);
      emit(state.copyWith(
        isLoadingMore: false,
        hasError: true,
  ...
  void _onRefreshTasks(RefreshTasksEvent event, Emitter<TaskListState> emit) {
    ...
    emit(TaskListState.initial().copyWith(
  ...
  void _onTasksStreamDataReceived(TasksStreamDataReceived event, Emitter<TaskListState> emit) {
    ...
    outcome.when(
      success: (resultPage) {
        ...
        final updatedTasks = <TaskModel>[...newTasks, ...existingTasksFromOtherPages];
      ...
      failure: (error, message, throwable) {
        ...
        emit(state.copyWith(
          ...
          showNotification: true,
          notificationMessage: message ?? 'Não foi possível carregar as tarefas',
          notificationType: NotificationType.error,
        ));
  ...
  void _onTasksPageDataReceived(TasksPageDataReceived event, Emitter<TaskListState> emit) {
    ...
    outcome.when(
      success: (resultPage) {
        if (resultPage == null) {
          log('[TaskListBloc] Received null result page for page ${event.page}');
          emit(state.copyWith(
            isLoadingMore: false,
            hasMorePages: false,
          ));
        ...
        final newTasks = resultPage.items ?? [];
        log('[TaskListBloc] Page ${event.page} loaded ${newTasks.length} tasks');
      ...
      failure: (error, message, throwable) {
        log('[TaskListBloc] Page ${event.page} error: $error, message: $message');
