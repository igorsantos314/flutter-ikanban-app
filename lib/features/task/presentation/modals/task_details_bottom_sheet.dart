...
class TaskDetailsBottomSheet extends StatelessWidget {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (task.shouldNotify) ...[
                      ...
                      if (task.notifyMinutesBefore != null &&
                          task.notifyMinutesBefore! > 0) ...[
                        const SizedBox(height: 8),
                        Container(
  ...
  String _getNotificationTimeText(int minutesBefore) {
    if (minutesBefore == 0) {
    ...
    if (hours > 0 && minutes > 0) {
      return '$hours hora${hours > 1 ? 's' : ''} e $minutes minuto${minutes > 1 ? 's' : ''} antes';
    } else if (hours > 0) {
  ...
  String _getScheduledNotificationText(
    DateTime dueDate,
    DateTime dueTime,
    int minutesBefore,
  ) {
    ...
    if (notificationTime.isBefore(now)) {
      return 'O horário da notificação já passou';
    }

    return 'Notificação agendada para ${DateFormat('dd/MM/yyyy HH:mm').format(notificationTime)}';
  }
